...
impl Round0 {
        ...
        O: Push<Msg<(MessageA, SignBroadcastPhase1)>>,
    {
        let sign_keys = SignKeys::create(
            ...
            self.s_l[self.i as usize - 1] - 1,
            &self.s_l.iter().map(|&i| i - 1).collect::<Vec<_>>(),
        );
...
impl Round2 {
        ...
        O: Push<Msg<(DeltaI, TI, TIProof)>>, // TODO: unify TI and TIProof
    {
        ...
        let l_s: Vec<_> = self.s_l.iter().cloned().map(|i| i - 1).collect();
        ...
        Ok(Round3 {
            ...
            sign_keys: self.sign_keys.clone(),
            m_a: self.m_a.clone(),
            mb_gamma_s: m_b_gamma_s,
            bc_vec: self.bc_vec.clone(),
            ...
            sigma_i,
            t_i_proof,
            phase1_decom: self.phase1_decom.clone(),
        })
    }
        ...
        O: Push<Msg<(SI, HEGProof)>>,
    {
        ...
        for i in 0..ttag {
            LocalSignature::phase5_verify_pdl(
                ...
                &self.R,
                &self.m_a_vec[i].c,
                &self.local_key.paillier_key_vec[l_s[i] as usize],
