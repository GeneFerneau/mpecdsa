...
use crate::party_i::{self, KeyGenBroadcastMessage1, KeyGenDecommitMessage1, Keys};
use crate::ErrorType;
...
impl Round0 {
    ...
    where
        O: Push<Msg<party_i::KeyGenBroadcastMessage1>>,
    {
...
impl Round1 {
    ...
    where
        O: Push<Msg<party_i::KeyGenDecommitMessage1>>,
    {
        output.push(Msg {
            sender: self.party_i.clone(),
            receiver: None,
...
pub struct Round3 {
    ...
    bc_vec: Vec<party_i::KeyGenBroadcastMessage1>,
...
impl Round4 {
    pub fn proceed(self, input: BroadcastMsgs<DLogProof<Secp256k1, Sha256>>) -> Result<LocalKey> {
        let params = party_i::Parameters {
            threshold: self.t.into(),
            share_count: self.n.into(),
        };
...
#[serde(bound(serialize = "Point<Secp256k1>: Serialize, Scalar<Secp256k1>: Serialize"))]
#[serde(bound(
    deserialize = "Point<Secp256k1>: Deserialize<'de>, Scalar<Secp256k1>: Deserialize<'de>"
))]
pub struct LocalKey {
    pub paillier_dk: paillier::DecryptionKey,
    pub pk_vec: Vec<Point<Secp256k1>>,
    pub keys_linear: party_i::SharedKeys,
