...
impl GlobalStatePhase5 {
    ...
    pub fn phase5_blame(&self) -> Result<(), ErrorType> {
        ...
        for i in 0..len {
            if self.g_gamma_vec[i] != &generator * &self.gamma_vec[i] {
            ...
            .map(|i| {
                ...
                let alpha_beta_vector = (0..len - 1)
                    .map(|j| {
                        ...
                        let alpha = k_i_gamma_j.sub(&beta);
...
impl GlobalStatePhase6 {
    pub fn extract_paillier_randomness(ciphertext: &BigInt, dk: &DecryptionKey) -> BigInt {
        let raw_c = RawCiphertext::from(ciphertext);
                ...
                .map(|i| {
                    (0..len - 1)
                        .map(|j| {
                            ...
                            let miu = Scalar::<Secp256k1>::from(&self.miu_vec[i][j]);
                            let g_miu = &generator * &miu;
            ...
            let mut g_sigma_i_vec = (0..len)
                .map(|i| {
                    ...
                    let sum = self.miu_vec[i].iter().fold(g_wi_ki, |acc, x| {
                        acc + (&generator * &Scalar::<Secp256k1>::from_bigint(&x))
                    });
                    sum
                })
                .collect::<Vec<Point<Secp256k1>>>();
