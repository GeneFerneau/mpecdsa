...
use crate::utilities::{create_hash, zk_pdl_with_slack::PDLwSlackProof};
use crate::ErrorType;
...
fn sign(
    ...
    s: Vec<u16>, //participant list indexed from zero
    corrupt_step: usize,
    ...
    let sign_keys_vec = (0..ttag)
        .map(|i| SignKeys::create(&private_vec[s[i] as usize], &vss_scheme, s[i], &s))
    ...
    let m_a_vec: Vec<_> = sign_keys_vec
        .iter()
        .enumerate()
        .map(|(i, k)| MessageA::a(&k.k_i, &party_keys_vec[s[i] as usize].ek))
        .collect();
    ...
    for i in 0..ttag {
        ...
        for j in 0..ttag - 1 {
            ...
            let alpha_ij_gamma = m_b
                .verify_proofs_get_alpha(&party_keys_vec[s[i] as usize].dk, &sign_keys_vec[i].k_i)
    ...
    let R_vec = (0..ttag)
        .map(|i| {
            ...
            SignKeys::phase4(&delta_inv, &b_proof_vec, &decommit_vec1, &bc1_vec, i).expect("")
    ...
    for i in 0..ttag {
        for j in 0..ttag - 1 {
            ...
            let proof = LocalSignature::phase5_proof_pdl(
                ...
                &sign_keys_vec[i].k_i,
                &m_a_vec[i].1,
                &dlog_statement_vec[s[ind] as usize],
            );
    ...
    if phase5_check.is_err() {
        ...
        for i in 0..ttag {
            ...
            let local_state = LocalStatePhase5 {
                ...
                encryption_key: ek_vec[s[i] as usize].clone(),
            };
        ...
        let ek_vec = (0..ttag)
            .map(|k| ek_vec[s[k] as usize].clone())
            .collect::<Vec<EncryptionKey>>();
        ...
        for i in 0..ttag {
            ...
            for j in 0..ttag - 1 {
                let rand = GlobalStatePhase6::extract_paillier_randomness(
                    &m_b_w_vec_all[i][j].c,
                    &party_keys_vec[s[i] as usize].dk,
                );
    ...
    let message_bn = create_hash(&[&BigInt::from_bytes(&message[..])]);
