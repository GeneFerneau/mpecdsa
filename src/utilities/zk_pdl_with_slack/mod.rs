...
use crate::utilities::create_hash;
...
impl PDLwSlackProof {
    pub fn prove(witness: &PDLwSlackWitness, statement: &PDLwSlackStatement) -> Self {
        ...
        let e = create_hash(&[
            &BigInt::from_bytes(&statement.G.to_bytes(true)),
            &BigInt::from_bytes(&statement.Q.to_bytes(true)),
            &statement.ciphertext,
            &z,
            &BigInt::from_bytes(&u1.to_bytes(true)),
            &u2,
            &u3,
        ]);
    ...
    pub fn verify(&self, statement: &PDLwSlackStatement) -> Result<(), ()> {
        let e = create_hash(&[
            ...
            &statement.ciphertext,
            &self.z,
            &BigInt::from_bytes(&self.u1.to_bytes(true)),
            &self.u2,
            &self.u3,
        ]);
        let g_s1 = statement.G.clone() * &Scalar::<Secp256k1>::from(&self.s1);
        let e_fe_neg = Scalar::<Secp256k1>::from(&(Scalar::<Secp256k1>::group_order() - &e));
